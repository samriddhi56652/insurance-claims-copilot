...
class KnowledgeBaseService:
    ...
    def _build_embedding_function(self) -> Any:
        if self._settings.google_api_key:
            # Chroma's GoogleGenaiEmbeddingFunction reads its key from GEMINI_API_KEY
            # (api_key_env_var default). Setting GOOGLE_API_KEY as well makes the
            # google-genai client emit a "both are set" warning, so use only this one.
            ...
            try:
                ...
                raise RuntimeError(
                    f"Gemini embedding initialization failed ({exc}). "
                    "Verify GOOGLE_API_KEY is a valid AI Studio key."
                ) from exc
